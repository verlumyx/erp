...
export function AdjustmentList({
    ...
    const [filters, setFilters] = useState<AdjustmentFilters>(initialFilters);
    ...
    const applyFilters = (next: AdjustmentFilters) => {
        setFilters(next);
        router.get(
            adjustments.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(adjustments.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="rounded-lg border bg-card p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="filter-count">Conteo</Label>
                        <Input
                            id="filter-count"
                            type="text"
                            placeholder="Identificador del conteo..."
                            value={filters.count_id ?? ''}
                            onChange={(e) =>
                                setFilters({
                                    ...filters,
            ...
            <Card className="gap-0 overflow-hidden rounded-2xl py-0">
                ...
                <div className="flex flex-col">
                    {rows.map((row) => (
                        ...
                        >
                            ...
                            <div
                                className={`hidden text-[13.5px] font-semibold tabular-nums lg:block ${
                                    Number(row.net_cost) < 0
                                        ? 'text-bad'
                                        : Number(row.net_cost) > 0
                                          ? 'text-ok'
                                          : ''
                            ...
                            >
                                {formatAmount(Number(row.net_cost), currency)}
                            </div>
                            <div className="hidden lg:block">
                ...
                <div className="px-5 py-3.5 text-[13px] font-semibold text-muted-foreground">
                    {rows.length} de {meta.total} ajuste
                    {meta.total !== 1 ? 's' : ''}
                </div>
            </Card>
        </div>
