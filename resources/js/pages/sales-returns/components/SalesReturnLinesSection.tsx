...
import { lineAmounts } from '../hooks/useSalesReturnForm';
import {
    CONDITION_LABELS,
    formatAmount,
    REASON_LABELS,
...
/** Valor del select cuando la línea hereda la condición de la cabecera. */
const HEADER_CONDITION = 'header';
 ...
 * precio al que se vendió; el importe que se muestra es el mismo que recalcula
 * el backend al guardar. El costo con el que la mercancía reingresa no se
 * captura: lo copia el backend de la venta original.
 */
export function SalesReturnLinesSection() {
    const {
        data,
        errors,
        catalog,
     ...
     * apartarse de la condición de la cabecera para destruirse: esa no
     * reingresa a ninguna parte.
     */
    const conditionOptions: OptionType[] = [
        { value: HEADER_CONDITION, label: 'El de la devolución' },
        ...(data.condition === 'scrap'
            ? []
            : [
                  {
                      value: 'scrap',
                      label: CONDITION_LABELS.scrap,
                  },
              ]),
    ];
        ...
        <div className="flex flex-col gap-4 p-5">
            ...
            {data.lines.map((line, index) => {
                ...
                const hasCharges =
                    ...
                    line.reason !== '' ||
                    line.condition !== '';
                const remaining = remainingOf(line.sales_invoice_line_id);
                                            ...
                                            updateLine(
                                                ...
                                                !option ||
                                                    option.value ===
                                                        HEADER_REASON
                                                    ? ''
                                                    : (option.value as SalesReturnReason),
                                            )
                                        }
                                            ...
                                            updateLine(
                                                ...
                                                !option ||
                                                    option.value ===
                                                        HEADER_CONDITION
                                                    ? ''
                                                    : (option.value as SalesReturnCondition),
                                            )
                                        }
                                        error={!!fieldError(index, 'condition')}
                                        size="md"
                                        placeholder="El de la devolución"
                                    />
                                    <span className="text-[12px] text-muted-foreground">
                                        Lo que se destruye no reingresa al
                                        inventario
                                    </span>
                                    {fieldError(index, 'condition') && (
                                        <p className="text-sm text-bad">
                                            {fieldError(index, 'condition')}
                                        </p>
                                    )}
