...
const CONDITION_OPTIONS: OptionType[] = [
    { value: '', label: 'Todos' },
...
export function SalesReturnList({
...
}: SalesReturnListProps) {
    const { currentCompany } = usePage<PageProps>().props;
    const companyId = currentCompany!.id;

    const [filters, setFilters] = useState<SalesReturnFilters>(initialFilters);

    /**
     * El filtro de cliente busca contra el servidor, así que de un id que
    ...
    const applyFilters = (next: SalesReturnFilters) => {
        setFilters(next);
        router.get(
            salesReturns.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(salesReturns.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
                ...
                <Button
                    ...
                    onClick={() =>
                        router.visit(salesReturns.create(companyId).url)
                    }
                >
                    <Plus />
                    Nueva devolución
