...
export interface SalesCreditNoteLineRow {
    id: string;
    item_id: string;
    ...
    /** Solo hace falta cuando la mercancía reingresa al inventario. */
    warehouse_id: string;
    notes: string;
}

interface SalesCreditNoteFormData {
    id: string;
    client_id: string;
...
/**
 * El cliente de la nota que se edita, con la etiqueta que trae su Resource: así
 * el select lo muestra desde el primer render.
 */
function clientSeed(note?: SalesCreditNote): AjaxOption | null {
    if (!note?.client_id) {
        return null;
    }
    ...
    return {
        ...
        label: note.client_code ? `${note.client_code} — ${name}` : name,
    };
}

/** La factura que la nota corrige, si corrige alguna. */
function invoiceSeed(note?: SalesCreditNote): AjaxOption | null {
    if (!note?.sales_invoice_id) {
        return null;
    }

    const code = note.sales_invoice_code ?? 'Factura de venta';

    return {
        value: note.sales_invoice_id,
        label: note.sales_invoice_number
            ? `${code} · ${note.sales_invoice_number}`
            : code,
    };
}
...
export interface SalesCreditNoteTotals {
    ...
    subtotal: number;
    taxAmount: number;
    /** Parte del impuesto que el cliente retiene en vez de pagarla. */
    withholdingAmount: number;
    total: number;
}

function emptyLine(): SalesCreditNoteLineRow {
    return {
        id: generateUUID(),
        item_id: '',
        measurement_unit_id: '',
 ...
 */
function lineRows(note?: SalesCreditNote): SalesCreditNoteLineRow[] {
...
/**
 * Espejo del cálculo del backend (`SalesCreditNoteLineData`): sirve para
 * mostrar el resumen mientras se captura. El importe que se guarda siempre lo
 * recalcula el servidor.
 */
export function lineAmounts(line: SalesCreditNoteLineRow): {
    gross: number;
    discountAmount: number;
    subtotal: number;
    taxAmount: number;
...
export function useSalesCreditNoteForm({
    mode,
    options,
    initialData,
    onSuccess,
}: UseSalesCreditNoteFormProps) {
    ...
    const { data, setData, post, put, transform, processing, errors, reset } =
        useForm<SalesCreditNoteFormData>({
            id: initialData?.id ?? generateUUID(),
            client_id: initialData?.client_id ?? '',
            sales_invoice_id: initialData?.sales_invoice_id ?? '',
            note_series: initialData?.note_series ?? '',
            note_date:
     ...
     * precios ya capturados no se tocan: son los que se le facturaron.
     */
     ...
     * ella la trazabilidad de las líneas.
     */
    ...
    const updateLine = <K extends keyof SalesCreditNoteLineRow>(
        index: number,
        field: K,
        value: SalesCreditNoteLineRow[K],
    ) =>
        setData(
            'lines',
    ...
    const totals: SalesCreditNoteTotals = data.lines.reduce(
        (accumulator, line) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (mode === 'create') {
            post(salesCreditNotes.store(companyId).url, {
                onSuccess: () => {
                    reset();
                    onSuccess?.();
                },
