...
import {
    ...
    type SalesCreditNoteFilters,
    type SalesCreditNoteMeta,
    type SalesCreditNoteStatus,
} from '../types/SalesCreditNote';

interface SalesCreditNoteListProps {
...
export function SalesCreditNoteList({
    salesCreditNotes: rows,
    meta,
    filters: initialFilters,
}: SalesCreditNoteListProps) {
    const { currentCompany } = usePage<PageProps>().props;
    const companyId = currentCompany!.id;

    const [filters, setFilters] =
        useState<SalesCreditNoteFilters>(initialFilters);

    /**
     * El filtro de cliente busca contra el servidor, así que de un id que llega
     * en la URL solo sabemos su etiqueta si alguna nota listada lo nombra; el
    ...
    const client = useRemoteOption({
        ...
        seed: initialFilters.client_id
            ? {
                  ...
                  label:
                      rows.find(
                          (note) => note.client_id === initialFilters.client_id,
                      )?.client_name ?? 'Cliente',
              }
            : null,
        hydrate: true,
    ...
    const applyFilters = (next: SalesCreditNoteFilters) => {
        setFilters(next);
        router.get(
            salesCreditNotes.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(salesCreditNotes.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
                ...
                <Button
                    ...
                    onClick={() =>
                        router.visit(salesCreditNotes.create(companyId).url)
                    }
                >
                    <Plus />
                    Nueva nota
            ...
            <div className="rounded-lg border bg-card p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="filter-number">Número fiscal</Label>
                        <Input
                            id="filter-number"
                            type="text"
                            placeholder="Correlativo de la nota..."
                            ...
                            >
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        ...
                                        {isEditable(
                                            note.status as SalesCreditNoteStatus,
                                        ) && (
                                            <DropdownMenuItem
                                                onClick={() =>
                                                    router.visit(
