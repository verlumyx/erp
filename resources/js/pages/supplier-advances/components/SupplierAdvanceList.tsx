...
import {
    ...
    type SupplierAdvanceMeta,
} from '../types/SupplierAdvance';

interface SupplierAdvanceListProps {
...
export function SupplierAdvanceList({
    supplierAdvances: rows,
    meta,
    filters: initialFilters,
}: SupplierAdvanceListProps) {
    const { currentCompany } = usePage<PageProps>().props;
    const companyId = currentCompany!.id;

    const [filters, setFilters] =
        useState<SupplierAdvanceFilters>(initialFilters);

    /**
     * El filtro de proveedor busca contra el servidor, así que de un id que
     * llega en la URL solo sabemos su etiqueta si algún anticipo listado lo
    ...
    const applyFilters = (next: SupplierAdvanceFilters) => {
        setFilters(next);
        router.get(
            supplierAdvances.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(supplierAdvances.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
                ...
                <Button
                    ...
                    onClick={() =>
                        router.visit(supplierAdvances.create(companyId).url)
                    }
                >
                    <Plus />
                    Nuevo anticipo
