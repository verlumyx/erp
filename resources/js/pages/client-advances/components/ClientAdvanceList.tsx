...
import {
    ...
    STATUS_PILL_KIND,
    type ClientAdvance,
    type ClientAdvanceFilters,
    type ClientAdvanceMeta,
} from '../types/ClientAdvance';

interface ClientAdvanceListProps {
...
export function ClientAdvanceList({
    clientAdvances: rows,
    meta,
    filters: initialFilters,
}: ClientAdvanceListProps) {
    const { currentCompany } = usePage<PageProps>().props;
    const companyId = currentCompany!.id;

    const [filters, setFilters] =
        useState<ClientAdvanceFilters>(initialFilters);

    /**
     * El filtro de cliente busca contra el servidor, así que de un id que llega
     * en la URL solo sabemos su etiqueta si algún anticipo listado lo nombra;
     * el resto lo resuelve la hidratación del propio select.
     */
        ...
        seed: initialFilters.client_id
            ? {
                  ...
                  label:
                      rows.find(
                          (advance) =>
                              advance.client_id === initialFilters.client_id,
                      )?.client_name ?? 'Cliente',
              }
            : null,
        hydrate: true,
    ...
    const applyFilters = (next: ClientAdvanceFilters) => {
        setFilters(next);
        router.get(
            clientAdvances.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(clientAdvances.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            ...
            <Card className="gap-0 overflow-hidden rounded-2xl py-0">
                    ...
                    {rows.map((advance) => (
                        ...
                        >
                            ...
                            <div className="hidden text-[13.5px] font-semibold tabular-nums lg:block">
                                {advance.currency} {advance.amount}
                            </div>
                            ...
                            <div className="hidden lg:block">
                                <StatusPill
                                    kind={STATUS_PILL_KIND[advance.status]}
                                >
                ...
                <div className="px-5 py-3.5 text-[13px] font-semibold text-muted-foreground">
                    {rows.length} de {meta.total} anticipo
                    {meta.total !== 1 ? 's' : ''}
                </div>
            </Card>
        </div>
