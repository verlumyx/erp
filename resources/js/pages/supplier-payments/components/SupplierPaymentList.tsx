...
export function SupplierPaymentList({
...
}: SupplierPaymentListProps) {
    const { currentCompany } = usePage<PageProps>().props;
    const companyId = currentCompany!.id;

    const [filters, setFilters] =
        useState<SupplierPaymentFilters>(initialFilters);

    /**
     * El filtro de proveedor busca contra el servidor, así que de un id que
     * llega en la URL solo sabemos su etiqueta si algún pago listado lo nombra;
    ...
    const applyFilters = (next: SupplierPaymentFilters) => {
        setFilters(next);
        router.get(
            supplierPayments.index(companyId).url,
    ...
    const handleClear = () => {
        setFilters({});
        router.get(supplierPayments.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            ...
            <Card className="gap-0 overflow-hidden rounded-2xl py-0">
                ...
                <div className="flex flex-col">
                    {rows.map((payment) => (
                        ...
                        >
                            ...
                            <div className="hidden lg:block">
                                <StatusPill
                                    kind={STATUS_PILL_KIND[payment.status]}
                                >
                ...
                <div className="px-5 py-3.5 text-[13px] font-semibold text-muted-foreground">
                    {rows.length} de {meta.total} pago
                    {meta.total !== 1 ? 's' : ''}
                </div>
            </Card>
        </div>
