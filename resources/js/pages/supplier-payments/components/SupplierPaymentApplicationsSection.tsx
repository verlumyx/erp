...
/**
 * El reparto del pago entre las facturas abiertas del proveedor.
 *
 * No es una tabla de líneas que se agregan y se quitan: las filas son las
 * facturas que el proveedor tiene con saldo, y lo único que se captura es
 * cuánto se le abona a cada una.
 */
export function SupplierPaymentApplicationsSection() {
    const {
        data,
        errors,
        invoices,
    ...
    const rowError = (invoiceId: string): string | undefined => {
        ...
        return (
            ...
            fields[`applications.${index}.purchase_invoice_id`]
        );
    };
    ...
    if (data.supplier_id === '') {
        return (
            <div className="p-8 text-center text-sm text-muted-foreground">
                Elige antes el proveedor: el reparto se arma con sus facturas
                abiertas.
            </div>
        );
    }
    ...
    if (invoices.length === 0) {
        return (
            <div className="p-8 text-center text-sm text-muted-foreground">
                Este proveedor no tiene facturas con saldo. El pago quedará
                entero sin aplicar.
            </div>
        );
    }
