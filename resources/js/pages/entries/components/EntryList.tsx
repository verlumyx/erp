...
const INSPECTION_OPTIONS: OptionType[] = [
    { value: ALL, label: 'Todos' },
    ...Object.entries(INSPECTION_LABELS).map(([value, label]) => ({
        value,
        label,
    })),
];
...
export function EntryList({
     ...
     * llega en la URL solo sabemos su etiqueta si alguna entrada listada lo
    ...
    const handleClear = () => {
        setFilters({});
        router.get(entries.index(companyId).url);
    };
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="rounded-lg border bg-card p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="filter-document">Remisión</Label>
                        <Input
                            id="filter-document"
                            type="text"
                            placeholder="Guía del proveedor..."
                            value={filters.supplier_document ?? ''}
                            onChange={(e) =>
                                setFilters({
                                    ...filters,
                                    supplier_document: e.target.value,
                                })
                            }
                            onKeyDown={(e) =>
                                e.key === 'Enter' && applyFilters(filters)
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="filter-inspection">Inspección</Label>
                        <Select2
                            inputId="filter-inspection"
                            options={INSPECTION_OPTIONS}
                            value={
                                INSPECTION_OPTIONS.find(
                                    (option) =>
                                        option.value ===
                                        (filters.inspection_status ?? ALL),
                                ) ?? null
                            }
                            onChange={(option) =>
                                applyFilters({
            ...
            <Card className="gap-0 overflow-hidden rounded-2xl py-0">
                ...
                <div className="px-5 py-3.5 text-[13px] font-semibold text-muted-foreground">
                    {rows.length} de {meta.total} entrada
                    {meta.total !== 1 ? 's' : ''}
                </div>
            </Card>
        </div>
