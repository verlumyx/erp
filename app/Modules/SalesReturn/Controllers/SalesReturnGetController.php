...
use App\Modules\SalesReturn\Resources\SalesReturnOptionResource;
use App\Modules\SalesReturn\Resources\SalesReturnResource;
use App\Modules\SalesReturn\Services\SalesReturnFindService;
use App\Modules\SalesReturn\Services\SalesReturnFormOptionsService;
use App\Modules\SalesReturn\Services\SalesReturnOptionSearchService;
use App\Modules\SalesReturn\Services\SalesReturnSearchService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Inertia\Inertia;
use Inertia\Response;

class SalesReturnGetController extends Controller
{
    /** Claves de filtro aceptadas por el listado. */
    private const FILTERS = [
        'code', 'client_id', 'sales_invoice_id', 'warehouse_id',
        'reason', 'condition', 'status', 'date_from', 'date_to',
    ];
    ...
    public function __construct(
        private readonly SalesReturnSearchService $searchService,
        private readonly SalesReturnFindService $findService,
        private readonly SalesReturnFormOptionsService $formOptionsService,
        private readonly SalesReturnOptionSearchService $optionSearchService,
    ) {}
    ...
    {
        abort_unless($request->user()?->hasPermission('sales-returns.list') ?? false, 403);
        ...
        $command = new SearchSalesReturnCommand(
            filters: [
        ...
        return response()->json([
            'data' => array_map(
                fn (SalesReturn $return): array => (new SalesReturnOptionResource($return))->resolve(),
                $result['data'],
            ),
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('sales-returns/index', [
            'salesReturns' => array_map(
                fn (SalesReturn $return): array => (new SalesReturnResource($return))->resolve(),
                $result['data'],
            ),
            'meta' => [
                'total' => $result['total'],
    ...
    {
        abort_unless($request->user()?->hasPermission('sales-returns.create') ?? false, 403);
    ...
    {
        abort_unless(request()->user()?->hasPermission('sales-returns.show') ?? false, 403);

        $model = $this->findService->execute($id, $company);

        return Inertia::render('sales-returns/show', [
            'salesReturn' => (new SalesReturnResource($model))->resolve(),
        ]);
    }

    public function edit(string $company, string $id): Response
    {
        abort_unless(request()->user()?->hasPermission('sales-returns.update') ?? false, 403);
