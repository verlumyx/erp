...
use App\Modules\PurchaseCreditNote\Resources\PurchaseCreditNoteOptionResource;
...
class PurchaseCreditNoteGetController extends Controller
{
    ...
    public function lookup(Request $request, string $company): JsonResponse
    {
        ...
        return response()->json([
            'data' => array_map(
                fn (PurchaseCreditNote $note): array => (new PurchaseCreditNoteOptionResource($note))->resolve(),
                $result['data'],
            ),
            'has_more' => $result['total'] > $command->offset + $command->limit,
        ]);
