...
class UpdateStatusClientAdvanceRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->hasPermission('client-advances.update-status') ?? false;
    }

    /**
     * La pantalla solo aprueba o anula: `confirmed` y la vuelta a `draft` los
     * decide el cobro espejo, y `partial` / `completed` las aplicaciones a
     * facturas.
     *
    ...
    {
        return [
            'status' => ['required', 'string', Rule::in(ClientAdvance::REQUESTABLE_STATUSES)],
        ];
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            $allowed = ClientAdvance::STATUS_TRANSITIONS[$advance->status] ?? [];
