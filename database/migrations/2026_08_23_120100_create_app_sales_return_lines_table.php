...
return new class extends Migration
{
    ...
    {
        Schema::create('app_sales_return_lines', function (Blueprint $table) {
            ...
            /** Lote y serie devueltos: los mismos que se despacharon. */
            $table->uuid('lot_id')->nullable();
            $table->uuid('serial_id')->nullable();

            /** Ubicación de reingreso. Vacía usa la de por defecto de la bodega. */
            $table->uuid('location_id')->nullable();

            $table->decimal('quantity', 18, 4);
            $table->decimal('base_quantity', 18, 4);

            /** Precio al que se vendió: es lo que acreditará la nota de crédito. */
            $table->decimal('unit_price', 18, 6);

            /**
             * Costo al que reingresa: el congelado en la venta original, no el
             * promedio vigente. Lo resuelve el backend, no se captura.
             */
            ...
            /** Condición específica de la línea; sin ella manda la de la cabecera. */
            $table->enum('condition', ['resalable', 'damaged', 'scrap'])->nullable();
            ...
            $table->unique(['sales_return_id', 'line_number'], 'app_sales_return_lines_number_unique');
        });
    }

    public function down(): void
    {
        Schema::table('app_sales_return_lines', function (Blueprint $table) {
        ...
        Schema::dropIfExists('app_sales_return_lines');
    }
};
