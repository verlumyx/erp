...
test('the note freezes the catalog rate without the form sending it', function () {
    ...
    $note = SalesCreditNote::find($payload['id']);
...
test('a note in another currency also freezes the company rate', function () {
    ...
    $payload = salesCreditNotePayload($client, $item, $unit, ['currency' => 'EUR']);
...
test('a note in a currency without a loaded rate is not issued', function () {
    ...
    expect(SalesCreditNote::find($payload['id']))->toBeNull();
});
...
test('the rate typed in the form is ignored when the company forbids correcting it', function () {
    ...
    $payload = salesCreditNotePayload($client, $item, $unit, ['exchange_rate' => 1]);

    actingAs($user)->withSession(['current_company_id' => $company->id])
        ->post(route('sales-credit-notes.store', ['company' => $company->id]), $payload)
        ->assertSessionHasNoErrors();

    expect((float) SalesCreditNote::find($payload['id'])->exchange_rate)->toBe(36.5);
});

test('the rate typed in the form wins when the company allows correcting it', function () {
    [$user, $company, $client, , $item, $unit] = salesCreditNoteScenario();

    $payload = salesCreditNotePayload($client, $item, $unit, ['exchange_rate' => 38.25]);
...
test('an empty rate is not a manual correction', function () {
    ...
    $payload = salesCreditNotePayload($client, $item, $unit, ['exchange_rate' => '']);
