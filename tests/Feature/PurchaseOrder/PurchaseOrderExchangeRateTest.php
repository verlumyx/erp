...
/**
 * La tasa de una orden de compra sale del catálogo, no del formulario. Antes
 * bastaba con escribir 1 para registrar una compra en euros valorada como si
 * fuera en dólares.
 */
test('the order freezes the catalog rate without the form sending it', function () {
    ...
    $order = PurchaseOrder::find($payload['id']);
...
test('an order in another currency also freezes the company rate', function () {
    ...
    $payload = purchaseOrderPayload($supplier, $warehouse, $item, $unit, ['currency' => 'EUR']);
...
test('an order in a currency without a loaded rate is not issued', function () {
    ...
    expect(PurchaseOrder::find($payload['id']))->toBeNull();
});
...
test('the rate typed in the form is ignored when the company forbids correcting it', function () {
    ...
    $payload = purchaseOrderPayload($supplier, $warehouse, $item, $unit, ['exchange_rate' => 1]);

    actingAs($user)->withSession(['current_company_id' => $company->id])
        ->post(route('purchase-orders.store', ['company' => $company->id]), $payload)
        ->assertSessionHasNoErrors();

    expect((float) PurchaseOrder::find($payload['id'])->exchange_rate)->toBe(36.5);
});

test('the rate typed in the form wins when the company allows correcting it', function () {
    [$user, $company, $supplier, $warehouse, $item, $unit] = purchaseOrderScenario();

    $payload = purchaseOrderPayload($supplier, $warehouse, $item, $unit, ['exchange_rate' => 38.25]);

    actingAs($user)->withSession(['current_company_id' => $company->id])
        ->post(route('purchase-orders.store', ['company' => $company->id]), $payload)
        ->assertSessionHasNoErrors();

    expect((float) PurchaseOrder::find($payload['id'])->exchange_rate)->toBe(38.25);
});
...
test('saving the draft again refreshes the rate', function () {
    ...
    actingAs($user)->withSession(['current_company_id' => $company->id])
        ->put(
            route('purchase-orders.update', ['company' => $company->id, 'id' => $payload['id']]),
            purchaseOrderPayload($supplier, $warehouse, $item, $unit),
        )
        ->assertSessionHasNoErrors();

    expect((float) PurchaseOrder::find($payload['id'])->exchange_rate)->toBe(37.8);
});
...
test('an empty rate is not a manual correction', function () {
    ...
    $payload = purchaseOrderPayload($supplier, $warehouse, $item, $unit, ['exchange_rate' => '']);
